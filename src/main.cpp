...
/*
 * car_ahead: Determine if there is a vehicle within 30m ahead of the vehicle
 * car_right: Determine if there is a vehicle within 30m in the right lane
 * car_left: Determine if there is a vehicle within 30m in the left lane

 * my_lane: which lane I am
 
 * closest_ahead_dist: distance to the closet front car
 * closest_rightfront_dist: distance to the cloesest right-front vehicle
 * closest_rightback_dist: distance to the cloesest right-back vehicle
 * closest_leftfront_dist: distance to the cloesest left-front vehicle
 * closest_leftback_dist: distance to the cloesest left-back vehicle
 
 * cost_collision: cost for possible collision
 * right_turn_cost: cost for changing to right lane
 * left_turn_cost: cost for changing to left lane
 * slow_down_cost: cost for changing to slow down
 */
...
// unit conversion: mph/ms=3600.0/1600.0
double ms_to_mph(double ms) {
    return ms * (3600.0 / 1600.0);
}

// calculate cost for keeping the current lane
double keep_lane_cost(bool car_ahead, int my_lane, double closest_ahead_dist, int cost_collision) {
    double cost=0;
    if (car_ahead) {
        cost = cost_collision;
    } else {
        ...
        if (closest_ahead_dist >= 150) {// If there are no cars within 150 m, the chance of a collision is very small
            cost += 0;
        } else {
            cost += 100;
        }
...
// calculate cost for changing to the right lane
...
// calculate cost for changing to the left lane
...
// calculate cost for slowing down
...
//                        fusion_s+= ((double)prev_size*0.02*fusion_speed); // Does this work?
                        ...
                        if (others_lane == my_lane) {// in my lane
                            ...
                            if (dist > 0) {
                                closest_ahead_dist = min(dist,
                                                         closest_ahead_dist); // find the minimal distance to the closest front car
                            }
                    ...
                    cout << "My lane(0:left,1:center,2:right): " << my_lane << endl;
                    cout << "Dist to the car ahead(m):         " << closest_ahead_dist << endl;
                    cout << "Speed of the car ahead(m/s):      " << closest_front_vec << endl;
                    cout << "Dist to the car front left(m):    " << closest_leftfront_dist << endl;
                    cout << "Dist to the car front right(m):   " << closest_leftback_dist << endl;
                    cout << "Dist to the car rear left(m):     " << closest_rightfront_dist << endl;
                    cout << "Dist to the car rear right(m):    " << closest_rightback_dist << endl;
                    cout << "Lateral dist to the left car :    " << closest_left_d << endl;
                    cout << "Lateral dist to the right car :   " << closest_right_d << endl;
                    ...
                    keep_lane_c = keep_lane_cost(car_ahead, my_lane, closest_ahead_dist, cost_collision);
                    ...
                    cout << "Cost of keeping the lane:      " << keep_lane_c << endl;
                    cout << "Cost of slowing down:          " << slow_down_c << endl;
                    cout << "Cost of changing to the left:  " << turn_left_c << endl;
                    cout << "Cost of changing to the right: " << turn_right_c << endl;
                    ...
                    if (decision == 0) {
                        cout << "Decision = 0: Continue at the max velocity " << endl;
                    } else if (decision == 1) {
                        cout << "Decision = 1: Change to the right lane " << endl;
                    } else if (decision == 2) {
                        cout << "Decision = 2: Change to the left lane " << endl;
                    } else if (decision == 3) {
                        cout << "Decision = 3: Continue at lower velocity " << endl;
                    }
